...
def findAdditionalControls(controls,
    ...
    for control in controls:
        ...
        if hlist:
            ...
            for h in hlist:
                hdict[h]=''
            resultsList.append(list(hdict.keys()))
...
LINE_TEXT_BUFFER = array.array('c', b' ' * (LINE_BUFFER_LENGTH))
bufferlength  = struct.pack('i', LINE_BUFFER_LENGTH) # This is a C style int.
LINE_TEXT_BUFFER = array.array('c', bufferlength + b' ' * LINE_BUFFER_LENGTH)
...
def splitKeysString(keysString):
    ...
    """
    L = [_f for _f in hasBraces.split(keysString) if _f]
    M = []
