__version__ = "$Revision: 423 $, $Date: 2011-04-04 17:44:58 +0200 (ma, 04 apr 2011) $, $Author: quintijn $"
...
Also automatic repeating of commands implemented in this grammar.
...
<searching> exported = (<changeSearching>|<speed>|<searchDir>|<reverse>|
                      ...
                      <endSearching>;
<repeating> exported = <changeRepeating>|<endRepeating>;
...
<changeRepeating> = <acceleration>|<speed>;
...
<endRepeating> = (stop [herhaal|herhalen|halen]) | OK | wacht | okee| 'okee hallo';
...
<startSearching> exported = Search continue [<searchDir>|<speed>|<moveCount>]+;
    ...
    def initialize(self):
        debugPrint('init grammar "_repeat" language: %s' % language)
    ...
    def cancelMode(self):
        ...
        if self.inTimer or self.state == "repeat": 
            self.setTrayIcon()
        ...
        if self.repeatFlag is None:
            # first time and after exclusive mode:
            self.state = ''
            self.minorState = ''
            self.curDir = ''
            self.Count = 0
...
##                   (self.state, self.curDir, self.Count))
    ...
    def gotBegin(self,moduleInfo):
        ...
        if self.inTimer:
            self.inside = 1
        #debugPrint("gotBegin, lastResults: %s"% self.lastResults)
    ...
    def gotResultsObject(self,recogType,resObj):
        ...
        if recogType == 'reject':
            debugPrint("reject recognition, do nothing")
            return
        handle = natlink.getCurrentModule()[2]
        if handle != self.prevHandle:
            # new window, empty lastResul/ts list:
            debugPrint("new handle, reset previous stuff: %s"% handle)
            self.prevHandle = handle
            self.cancelMode()
        ...
        if recogType == 'other':
            ...
            if self.repeatFlag:
                # the repeatStuff was mimiced and should be there now, if not cancel
                if words != self.repeatStuff:
                    debugPrint("should not come here, other recognition, while repeatFlag is on: words: %s, repeatStuff: %s"% (words, self.repeatStuff))
                    self.cancelMode()
                    if words:
                ...
                if self.inTimer:
                    self.cancelMode()
                #debugPrint("other recogn, NO repeatFlag: words: %s"% words)
...
##                self.startNow()
    ...
    def gotResults_startRepeating(self,words,fullResults):
        self.activate('repeating', noError=1)
        debugPrint('start repeating, active rules: %s'% self.activeRules)
...
##                return None
    ...
    def setTrayIcon(self,toggleIcon=None):
        #if natqh.getDNSVersion() >= 12:
        #    return # silently ignore this
        if toggleIcon is None:
            debugPrint("setTrayIcon, reset tray icon")
            natqh.setTrayIcon()
            return
...
##        print 'iconName: %s'% iconName
        try:            
            ...
            natqh.setTrayIcon()
        try:
            natqh.setTrayIcon(iconName, toolTip, comingFrom=self) # expecting a method onTrayIcon
...
## taken from GrammarX:
    def onTrayIcon(self,message):
        debugPrint("_repeat, clicked on tray icon...")
        natqh.setTrayIcon()
        self.cancelMode()

    def repeatNow(self):
        #debugPrint('repeatNow: %s times, %s phrases\n---(%s)' % (self.nTimes, self.nPhrase, repr(self.lastResults)))
        # put repeat mode ON:
        self.repeatFlag = 1
        itemrange = range(len(self.lastResults))
        if not itemrange:
            debugPrint("no itemrange, no repeat possible")
            action("ALERT")
            self.repeatFlag = 0
            self.cancelMode()
            self.DisplayMessage("<_repeat: nothing to repeat>")
            return
        ...
        for n in range(self.nTimes):
            for i in itemrange:
                ...
                #debugPrint("mimic: %s (excl: %s, cbd: %s)"% (self.repeatStuff, self.exclusive, natlink.getCallbackDepth()))
                try:
                    natlink.recognitionMimic(self.repeatStuff)
                    print 'Mimiced: %s'% self.repeatStuff
                except natlink.MimicFailed:
                    # try lowercase:
                    try:
                        self.lastResults[i] = [w.lower() for w in self.lastResults[i]]
                        self.repeatStuff = copy.copy(self.lastResults[i])
                        print 'repeatStuff: %s'% self.repeatStuff
                        
                        natlink.recognitionMimic(self.repeatStuff)
                        print 'mimiced: %s'% self.repeatStuff
                    except natlink.MimicFailed:
                        print "could not mimic: %s (type: %s), do cancelmode"% (self.repeatStuff, type(self.repeatStuff))
                        self.cancelMode()
                        action("ALERT")
                        text = '<_repeat: mimic failed cancel repeat>'
