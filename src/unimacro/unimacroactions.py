...
#
# Since 2020 it is on https://github.com/dictation-toolbox/unimacro,
# under the conditions of the dictation-toolbox
#
...
#  July 2020 (an adaptation of the actions.py module in Unimacro)
#
...
natlink grammar, the Unimacro grammar _control.py) is needed
to activate these functions by voice.
...
coreDir = os.path.normpath(os.path.join(thisDir, "..", "Natlink", "MacroSystem", "core"))
if not os.path.isdir(coreDir):
    print("invalid startup of unimacroactions, coreDir not a valid directory: ", coreDir)
    sys.exit()
if not coreDir in sys.path:
...
Also see https://qh.antenna.nl/unimacro/installation/inifilestrategy.html
---------
...
class UnimacroAction:
    def __init__(self, action, pauseBA=None, pauseBK=None,
        ...
        if not self.progInfo:
            self.progInfo = natqh.getProgInfo(self.modInfo)
        
    def __call__(self):
        """directly do an action when instance is called"""
        if self.action:
            self.doAction(self.action)
        else:
            if debug > 0: D('calling UnimacroAction without an action to perform!')

    def doAction(self, action):
        """central function, doing the action
        """
    ...
    def getMetaAction(self, a, sectionList=None, progInfo=None):
        if progInfo is None:
    ...
    def get_instance_from_progInfo(self, progInfo):
    ...
    def getPosition(self, name, prog=None):
    ...
    def startVoiceCoder(self, nTrie=2):
